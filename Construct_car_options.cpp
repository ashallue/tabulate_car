...
Carmichael numbers.
  ...
  This is the third attempt.  I was getting a multiplicity of functions, all with mostly the same code.
  I am going to try using preprocessor directives to create fewer versions.  See README for the various options.
...
#include "Construct_car_options.h"

using namespace std;

// default sets X to 2^(16) = 65536.
// Incremental sieve starts at 2
Construct_car_options::Construct_car_options(){
  B_car = 65536;
  B_sieve = 2 * B_car;
  ...
  // by default, all booleans are true
  // So only prime pre-products, all triples for each, trivial crossover
  prime_preproducts = true;
  preproduct_complete = true;
  trivial_crossover = true;

  // set pre-processor directives
  // Apparently I cannot use straight variables directly.
  #define prime_preproducts_d true

  //testing
  #if prime_preproducts == true
    cout << "first directive set to true\n";
  #endif

  #if prime_preproducts_d
    cout << "second directive set to true\n";
  #endif
...
int64 Construct_car_options::admissable(int64 P, int64* Pprimes, long Pprimes_len){
...
*/
vector<pair<int64, bigint>> Construct_car_options::preproduct_construction(int64* P, long P_len, int64* Pminus, long Pminus_len, int64 L){
...
*/
pair<int64, bigint> Construct_car_options::completion_check(int64 P, int64 Delta, int64 D, int64 L, int64* q_primes, long q_primes_len){
 ...
 */
void Construct_car_options::tabulate_car(int64 B, long processor, long num_threads, string cars_file, string admissable_file){
  int64* P_factors;
  long   P_factors_len;
 ...
 */
void Construct_car_options::tabulate_car_primeP(int64 B, long processor, long num_threads, string cars_file){
  int64* P_factors;
  long   P_factors_len;
  int64* Pminus_factors;
  long   Pminus_factors_len;
 ...
 */
void Construct_car_options::tabulate_car_primeP_crossover(int64 B, long processor, long num_threads, string cars_file){
  int64* P_factors;
  long   P_factors_len;
  int64* Pminus_factors;
  long   Pminus_factors_len;
 ...
 */
vector<pair<int64, bigint>> Construct_car_options::preproduct_crossover(int64* P, long P_len, int64* Pminus, long Pminus_len, int64 L){
  vector<pair<int64, bigint>> output;
  int64 p;     // stores a temp prime
  long e;      // stores a temp exponent
  int64 Delta_bound;  // stores upper bound on Delta to ensure it isn't too big
